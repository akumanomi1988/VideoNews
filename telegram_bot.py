...
async def category_selection_handler(update: Update, context: CallbackContext):
    ...
    settings = load_settings()
    news_client = NewsAPIProvider(api_key=settings['newsapi']['api_key'])
    ...
    if not news_cache["news"] or is_cache_expired():
        latest_news = news_client.get_latest_news(
            # country=settings['newsapi']['country'],
            language=settings['tts_edge']['language'],
            limit=settings['newsapi']['page_size'],
