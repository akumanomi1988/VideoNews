...
class NewsVideoProcessor:
    ...
    def process_latest_news_in_short_format(self, forze_topic: str):
        ...
        for topic in latest_news:
            ...
            article, phrases, title, description, tags, cover_text = self.article_generator.generate_article_and_phrases_short(topic['title'])
            if article == "":
                self.send_progress(f"⚠️ Skipped article generation for: {topic['title']}")
                continue

            self.send_progress("🎨 Generating cover image...")
            cover = self.fetch_related_media(phrases=cover_text, style=StylePreset.YOUTUBE_THUMBNAIL, max_items=1)[0]
            ImageHelper.enhance_thumbnail(cover, cover_text, Position.BOTTOM_CENTER, Style.BOLD, 2000, 5)
    ...
    def process_latest_news_in_long_format(self, forze_topic: str):
        ...
        for topic in latest_news:
            ...
            article, phrases, title, description, tags = self.article_generator.generate_article_and_phrases_long(topic['title'])
            if article == "":
                self.send_progress(f"⚠️ Skipped article generation for: {topic['title']}")
                continue

            self.send_progress("🎨 Generating cover image...")
            cover = self.fetch_related_media(phrases=title, style=StylePreset.YOUTUBE_THUMBNAIL, max_items=1, orientation=AspectRatio.LANDSCAPE)[0]
            ...
            self.send_progress("📝 Generating subtitles...")
            subtitle_path = self.stt.generate_subtitles(audio_path)
            ...
            media_manager = MediaManager(
                ...
                output_file=output_file,
                media_images=media_images,
                aspect_ratio='16:9'  # Usando la relación de aspecto para videos largos
            )
            ...
            self.send_progress("📤 Uploading to YouTube...")
            youtube_response = self.youtube_uploader.upload_long_video(
                output_file,
                title=title,
                thumbnail_path=cover,
                description=description + '\n' + topic['url'],
